...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-bigearthnet_ibot_s2_head_%j.log
#SBATCH --array=0-0
...
python \
  train_classifier.py \
  --experiment_name \
  TMLR_x-bigearthnet_ibot_s2_head_${seed} \
  --dataset_name \
  m_ben \
  --in_features \
  768 \
...
python \
  ...
  ./configs/m_ben.json \
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/classification/TMLR_x-bigearthnet_ibot_s2_head_${seed}/best-model-f1.ckpt \
  --img_size \
  224 \
  --filename \
  logs_ICLR/cls/TMLR_x-bigearthnet_ibot_s2_head_ \
  --bands \
