...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-eurosat_resnet-50_s2_head_%j.log
#SBATCH --array=0-0
...
python \
  train_classifier.py \
  --experiment_name \
  TMLR_x-eurosat_resnet-50_s2_head_${seed} \
  --dataset_name \
  m_eurosat \
  --in_features \
  2048 \
...
python \
  ...
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/classification/TMLR_x-eurosat_resnet-50_s2_head_${seed}/best-model.ckpt \
  --img_size \
  224 \
  --filename \
  logs_ICLR/cls/TMLR_x-eurosat_resnet-50_s2_head_ \
  --bands \
