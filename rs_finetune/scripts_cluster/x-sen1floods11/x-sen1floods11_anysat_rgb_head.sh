...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-sen1floods11_anysat_rgb_head_%j.log
#SBATCH --array=0-0
...
torchrun \
  ...
  $seed \
  --experiment_name \
  TMLR_x-sen1floods11_anysat_rgb_head_${seed} \
  --dataset_name \
  sen1floods11 \
  --dataset_path \
...
python \
  ...
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/segmentation/TMLR_x-sen1floods11_anysat_rgb_head_${seed}/best_model.pth \
  --size \
  224 \
  --classes \
  2 \
