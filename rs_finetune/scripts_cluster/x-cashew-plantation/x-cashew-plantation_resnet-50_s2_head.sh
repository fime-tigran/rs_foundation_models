...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-cashew-plantation_resnet-50_s2_head_%j.log
#SBATCH --array=0-0
...
torchrun \
  ...
  $seed \
  --experiment_name \
  TMLR_x-cashew-plantation_resnet-50_s2_head_${seed} \
  --dataset_name \
  cashew \
  --dataset_path \
...
python \
  ...
  ./configs/cashew.json \
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/segmentation/TMLR_x-cashew-plantation_resnet-50_s2_head_${seed}/best_model.pth \
  --size \
  224 \
  --classes \
  7 \
