...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-SA-crop-type_croma_rgb_full_%j.log
#SBATCH --array=0-0
...
torchrun \
  ...
  $seed \
  --experiment_name \
  TMLR_x-SA-crop-type_croma_rgb_full_${seed} \
  --dataset_name \
  crop \
  --dataset_path \
...
python \
  ...
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/segmentation/TMLR_x-SA-crop-type_croma_rgb_full_${seed}/best_model.pth \
  --size \
  120 \
  --classes \
  10 \
