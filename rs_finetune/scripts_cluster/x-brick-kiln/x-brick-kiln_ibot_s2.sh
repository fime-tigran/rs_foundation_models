...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-brick-kiln_ibot_s2_full_%j.log
#SBATCH --array=0-0
...
python \
  train_classifier.py \
  --experiment_name \
  TMLR_x-brick-kiln_ibot_s2_full_${seed} \
  --dataset_name \
  m_brick \
  --in_features \
  768 \
...
python \
  ...
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/classification/TMLR_x-brick-kiln_ibot_s2_full_${seed}/best-model.ckpt \
  --img_size \
  224 \
  --filename \
  logs_ICLR/cls/TMLR_x-brick-kiln_ibot_s2_full_ \
  --bands \
