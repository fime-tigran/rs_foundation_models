...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-so2sat_chivit_s2_full_%j.log
#SBATCH --array=0-0
...
python \
  train_classifier.py \
  --experiment_name \
  TMLR_x-so2sat_chivit_s2_full_${seed} \
  --dataset_name \
  so2sat \
  --in_features \
  768 \
...
python \
  ...
  ./configs/so2sat.json \
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/classification/TMLR_x-so2sat_chivit_s2_full_${seed}/best-model.ckpt \
  --img_size \
  224 \
  --filename \
  logs_ICLR/cls/TMLR_x-so2sat_chivit_s2_full_ \
  --bands \
