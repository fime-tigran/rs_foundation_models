...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-oscd_vit-b_s2_full_%j.log
#SBATCH --array=0-0
...
torchrun \
  ...
  train_change.py \
  --experiment_name \
  TMLR_x-oscd_vit-b_s2_full_${seed} \
  --mode \
  vanilla \
  --dataset_name \
  oscd \
...
python \
  ...
  ./configs/oscd.json \
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/change_detection/TMLR_x-oscd_vit-b_s2_full_${seed}/best_model.pth \
  --size \
  224 \
  --filename \
