...
#SBATCH --output=/nfs/ap/mnt/frtn/logs_anna/tmlr_x-harvey-flood_terrafm_s2_full_%j.log
#SBATCH --array=0-0
...
torchrun \
  ...
  train_change.py \
  --experiment_name \
  TMLR_x-harvey-flood_terrafm_s2_full_${seed} \
  --mode \
  vanilla \
  --dataset_name \
  harvey \
...
python \
  ...
  ./configs/harvey.json \
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/change_detection/TMLR_x-harvey-flood_terrafm_s2_full_${seed}/best_model.pth \
  --size \
  224 \
  --filename \
