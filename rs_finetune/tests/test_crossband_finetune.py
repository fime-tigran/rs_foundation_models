...
def test_channel_dropout_eval_preserves_dtype_device():
    cd = ChannelDropout(p=0.5, min_channels=1)
    cd.eval()
...
def test_adapt_conv3d_preserve_rgb_kernel_133():
    old = nn.Conv3d(3, 4, kernel_size=(1, 3, 3), padding=(0, 1, 1))
    torch.manual_seed(5)
    nn.init.xavier_uniform_(old.weight)
    new = adapt_rgb_conv3d_layer_to_multiband_preserve_rgb(old, new_in_channels=4)
    assert new.in_channels == 4
    assert torch.allclose(new.weight[:, :3], old.weight)
    expected_mean = old.weight.mean(dim=1, keepdim=True)
    assert torch.allclose(new.weight[:, 3:4], expected_mean)
...
@patch("builtins.print")
def test_chivit_tiny_pooling_modes(mock_print):
    torch.manual_seed(42)
    model_combined = chivit_tiny(
        in_chans=3,
        img_size=[224],
        patch_size=16,
        add_ch_embed=True,
        enable_sample=False,
        pooling_mode="cls+channel_mean",
    )
    state = model_combined.state_dict()
    model_cls = chivit_tiny(
        in_chans=3,
        img_size=[224],
        patch_size=16,
        add_ch_embed=True,
    ...
    model_channel_mean = chivit_tiny(
        in_chans=3,
        img_size=[224],
        patch_size=16,
        add_ch_embed=True,
    ...
    model_channel_mean.load_state_dict(state)
    x = torch.randn(2, 3, 224, 224)
    channel_idxs = [0, 1, 2]
    out_cls = model_cls(x, channel_idxs)
    out_cm = model_channel_mean(x, channel_idxs)
    out_comb = model_combined(x, channel_idxs)
    assert out_cls.shape == (2, 192)
    assert out_cm.shape == (2, 192)
    assert out_comb.shape == (2, 192)
    expected_combined = (out_cls + out_cm) / 2
    assert torch.allclose(out_comb, expected_combined, atol=1e-5)


@patch("builtins.print")
def test_chivit_hcs_sampled_channel_count_bounds(mock_print):
    model = chivit_tiny(
        in_chans=6,
        img_size=[224],
        patch_size=16,
        add_ch_embed=True,
        enable_sample=True,
        min_sample_channels=2,
    )
    model.train()
    x = torch.randn(1, 6, 224, 224)
    channel_idxs = list(range(6))
    counts: set[int] = set()
    for seed in range(80):
        torch.manual_seed(seed)
        random.seed(seed)
        pe = model.patch_embed(x, channel_idxs)
        c = int(pe.shape[2])
        counts.add(c)
        assert 2 <= c <= 6
    assert len(counts) > 1
...
@patch("builtins.print")
def test_chivit_min_sample_channels_no_reduction(mock_print):
    torch.manual_seed(42)
    model = chivit_tiny(
        in_chans=3,
        img_size=[224],
...
@patch("builtins.print")
def test_chivit_channel_gate_zeros_channel(mock_print):
    torch.manual_seed(42)
    model = chivit_tiny(
        in_chans=3,
        img_size=[224],
...
def test_channel_embed_l2_reg_formula():
    e = torch.randn(1, 12, 8, 1, 1)
    e0 = e.clone() + 1.0
    embed_reg = (e - e0.to(e.device)).pow(2).mean()
    assert embed_reg.ndim == 0
    assert embed_reg.item() > 0
